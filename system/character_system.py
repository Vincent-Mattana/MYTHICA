...
class ItemType(Enum):
    ...
    BOOTS = "Boots"
    WEAPON = "Weapon"
    RANGED_WEAPON = "Ranged Weapon"  # Bows, crossbows, etc.
    AMMUNITION = "Ammunition"  # Arrows, bolts, etc.
    POTION = "Potion"  # Health potions, etc.
    RING = "Ring"
    AMULET = "Amulet"
    
    def is_stackable(self) -> bool:
        """Return whether items of this type can be stacked."""
        return self in (ItemType.AMMUNITION, ItemType.POTION)

class Item:
    def __init__(self, name: str, item_type: ItemType, stat_bonuses: Dict[StatType, int] = None, stack_size: int = 1):
        self.name = name
        self.item_type = item_type
        self.stat_bonuses = stat_bonuses or {}
        self.stack_size = stack_size if self.item_type.is_stackable() else 1
        self.description = self._generate_description()
    
    def can_stack_with(self, other: 'Item') -> bool:
        """Check if this item can stack with another item."""
        if not self.item_type.is_stackable():
            return False
        return (self.item_type == other.item_type and
                self.name == other.name and
                self.stat_bonuses == other.stat_bonuses)
    
    def add_to_stack(self, count: int = 1) -> int:
        """Add items to this stack. Returns number of items that couldn't fit."""
        if not self.item_type.is_stackable():
            return count
        space = 20 - self.stack_size
        to_add = min(space, count)
        self.stack_size += to_add
        return count - to_add
    
    def remove_from_stack(self, count: int = 1) -> int:
        """Remove items from this stack. Returns number of items actually removed."""
        if not self.item_type.is_stackable():
            return 0
        to_remove = min(self.stack_size, count)
        self.stack_size -= to_remove
        return to_remove
    ...
    def is_ranged_weapon(self) -> bool:
        """Check if this item is a ranged weapon."""
        return self.item_type == ItemType.RANGED_WEAPON
    ...
    def can_equip_to_slot(self, slot: EquipmentSlot) -> bool:
        ...
        compatibility = {
            ...
            ItemType.RANGED_WEAPON: [EquipmentSlot.WEAPON_1, EquipmentSlot.WEAPON_2],
            ItemType.AMMUNITION: [],  # Ammunition is stored in inventory, not equipped
...
class CharacterStats:
    ...
    def randomize_starting_stats(self, total_points: int = 66):
...
class ItemGenerator:
    ...
    ITEM_NAMES = {
        ...
        ItemType.RANGED_WEAPON: ["Short Bow", "Long Bow", "Hunter's Bow", "Elven Bow", "War Bow"],
        ItemType.AMMUNITION: ["Wooden Arrows", "Iron Arrows", "Steel Arrows", "Elven Arrows", "Fire Arrows"],
        ItemType.POTION: ["Health Potion", "Healing Draught", "Life Elixir", "Vitality Potion", "Restoration Brew"],
...
class Character:
    def __init__(self, name: str = "Hero", skip_random_generation: bool = False):
        self.name = name
        self.level = 1
        self.experience = 0
        ...
        # Generate starting character (skip if being created through class system)
        if not skip_random_generation:
    ...
    def _update_equipment_bonuses(self):
        ...
        old_max_hp = self.max_hp if hasattr(self, 'max_hp') else 0
        new_max_hp = self._calculate_max_hp()
        
        if hasattr(self, 'current_hp') and old_max_hp > 0:
            # Maintain HP percentage when max changes
            hp_percentage = self.current_hp / old_max_hp
            self.current_hp = int(new_max_hp * hp_percentage)
        else:
            self.current_hp = new_max_hp
        
        self.max_hp = new_max_hp
        
        # Update max mana when stats change
        old_max_mana = self.max_mana if hasattr(self, 'max_mana') else 0
        new_max_mana = self._calculate_max_mana()
        
        if hasattr(self, 'current_mana') and old_max_mana > 0:
            # Maintain mana percentage when max changes
            mana_percentage = self.current_mana / old_max_mana
            self.current_mana = int(new_max_mana * mana_percentage)
        else:
    ...
    def _calculate_max_mana(self) -> int:
        """Calculate maximum mana based on Mana stat, level, and perks."""
        mana_stat = self.stats.get_total_stat(StatType.MANA)
        base_mana = 10  # Base mana at level 1
        mana_bonus = max(0, (mana_stat - 10) * 2)  # +2 mana per point above 10
        level_bonus = (self.level - 1) * 3  # +3 mana per level
        
        # Apply perk bonuses
        perk_mana_bonus = self.get_perk_bonus("max_mana")
        
        total_mana = base_mana + mana_bonus + level_bonus + int(perk_mana_bonus)
        return max(1, total_mana)
    ...
    def has_ammunition(self) -> bool:
        """Check if the character has ammunition for ranged weapons."""
    ...
    def get_ranged_attack_damage(self) -> Tuple[int, bool]:
        """Calculate ranged attack damage based on Dexterity, equipped weapons, and perks.
        Returns (damage, is_critical)."""
        # Base damage from Dexterity
        base_damage = max(1, self.stats.get_total_stat(StatType.DEXTERITY) // 2)
        
        # Add weapon damage bonuses
        weapon_bonus = 0
        weapon = self.equipment.get_equipped_item(EquipmentSlot.WEAPON_1)
        
        if weapon and weapon.is_ranged_weapon():
            weapon_damage = sum(weapon.stat_bonuses.values())
            
            # Apply bow-specific perk bonuses
            if self.get_perk_bonus("bow_damage") > 0:
                weapon_damage = int(weapon_damage * (1.0 + self.get_perk_bonus("bow_damage")))
            
            # Add perception bonus for accuracy
            perception_bonus = self.stats.get_total_stat(StatType.PERCEPTION) // 3
            weapon_damage += perception_bonus
        ...
        for item in self.inventory.items:
            if item.item_type == ItemType.AMMUNITION:
                ammo_bonus = sum(item.stat_bonuses.values())
                weapon_bonus += ammo_bonus
                break
        
        # Apply perk damage bonuses
        perk_damage_bonus = self.get_perk_bonus("damage")
        if perk_damage_bonus > 0:
            weapon_bonus = int(weapon_bonus * (1.0 + perk_damage_bonus))
        
        # Calculate critical hit chance
        crit_chance = 0.05  # Base 5% chance
        luck = self.stats.get_total_stat(StatType.LUCK)
        crit_chance += (luck - 10) * 0.01  # +1% per luck above 10
        
        # Weapon type modifiers
        if weapon:
            weapon_name = weapon.name.lower()
            if "dagger" in weapon_name:
                crit_chance += 0.05  # Daggers have higher crit chance
            elif "bow" in weapon_name:
                crit_chance += 0.03  # Bows have slightly higher crit chance
    ...
    def get_attack_damage(self) -> Tuple[int, bool]:
        """Calculate melee attack damage based on Strength, weapons, and perks.
        Returns (damage, is_critical)."""
